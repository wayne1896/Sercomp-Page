...
include 'UsuariosEmpleado.php';
$user = new User();
if(isset($_POST['forgotSubmit'])){
	//check whether email is empty
    if(!empty($_POST['emailempleado'])){
		//check whether user exists in the database
		$prevCon['where'] = array('correo_empleado'=>$_POST['emailempleado']);
		...
		if($prevUser > 0){
			...
			$conditions = array(
				'correo_empleado' => $_POST['emailempleado']
			);
			$data = array(
...
//datos del empleado su correo y nombre 
$id_empleado= $_POST['emailempleado'];

$sql_empleado=mysqli_query($con,"select * from empleado where correo_empleado='".$id_empleado."'");
$rw_empleado=mysqli_fetch_array($sql_empleado);
$nombrecompleto=$rw_empleado['nombre_empleado']." ".$rw_empleado['apellido_empleado'];
$correocliente=$rw_empleado['correo_empleado'];

$mail = new PHPMailer(true);
			
			
				if($update){
                    $resetPassLink = 'http://localhost/tesis/recuperacion/ReiniciarPasswordEmpleado.php?fp_code='.$uniqidStr;
    ...
    header("Location:../EnviarPasswordEmpleado.php");
